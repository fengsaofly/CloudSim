	...
	public void updateVmAvailableConfig(List<Vm> vmList)
	{
		...
		for(int index=0;index<curVmAvailableConfig.size();index++){
			...
			for(int vmId=0;vmId<tempAvailableConfig.length;vmId++){
				...
				tempAvailableConfig[vmId] =tempAvailableConfig[vmId] + curVmList[vmId];
...
//			timer = new Timer();
//			timer.schedule(this,(long)0, (long)GlobalParameter.SLOT);//延后三秒钟进行vm的分配
//			
//			timerStarted = true;
//		}
//		thread.start();
	...
	private void beginVMsAllocation(){
		...
		List<List<myVm>> waitVmsQueue1 = new ArrayList<List<myVm>>(waitVmsQueue.size());
...
//		}
	...
	public boolean getConfigFromMySchedulingAlgorithm(){
		...
		if(actualAvailableConfig.contains(new int[]{0,0,0}))  return false;
...
//				}
		...
		Log.printLine("Fly:当前主机"+this.getId()+"的cpu利用率为："+cpuShare+",mem利用率为："+memShare+"综合利用率为："+sysShare);
		//记录当前综合利用率
		curSynUtilization1 = sysShare;
		final int column = this.getId()*2;
		if(column>200) Log.printLine("column >200 error!");
...
//		}
	...
	private boolean getConfigFromStochasticSchedulingAlgorithm(){
		...
		double sysShare = (cpuShare+memShare)/2.0;
		Log.printLine("other:当前主机"+this.getId()+"的cpu利用率为："+cpuShare+",mem利用率为："+memShare+"综合利用率为："+sysShare);
...
//		Log.printLine("waitVmsQueue---"+processNum++);
		//可用配置为空
		if(curVmAvailableConfig==null || curVmAvailableConfig.size() ==0 || Arrays.equals(curVmAvailableConfig.get(0), new int[]{0,0,0}))
			{
		...
		if(!hasVmsForWaiting) return;//如果没有等待任务
		//如果有等待任务
		...
		//等待任务可以完成，或者完成一部分
		if(needConfig){
//			Log.printLine(this.getId()+"主机正在进行第"+(count++)+"次配置");
			beginVMsAllocation();
		}
...
//		  Log.print(" [ "+arrayResult+" ] ");
	...
	public void recoverHostConfig(Vm vm){
				...
				if(host!=null){
					...
					Log.printLine("主机"+host.getId()+"可用的配置已经更新!AvailableConfigs="+host.getCurVmAvailableConfig());
				}
	}
